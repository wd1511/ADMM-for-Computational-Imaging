function [out,tm,RMSE]=ADMM_MWPR(o,u0,op,z1,lambda,deltaA,deltaB,gamma,mu,a0,ITER,alpha)
...
[V,B,strs,tm]=TransformMatrix(method<1,z1,lambda,deltaA,deltaB,Nya,Nxa,K,gammar,mu,av);
...
for jndex = 1:ITER
    ...
    switch floor(op)
        case 0 % complex-valued
            ...
            u(bourdery,bourderx) = soft(u0_iter(bourdery,bourderx)-v(bourdery,bourderx),mu,gamma_scaler * gamma, 1);
        ...
        case 1 % AM
            ...
            PP = SF(Ur+Vr,B,gammar);
            u0_iter = ifft2(PP) + gamma * (u+v);
            u(bourdery,bourderx) = soft(u0_iter(bourdery,bourderx)-v(bourdery,bourderx),mu,gamma_sclaer * gamma, 1);
...
function [F,B,strs,t]=TransformMatrix(x,z1,lambda,deltaA,deltaB,N,M,K,gamma,mu,av)
...
for index = 1:K
    if x       
        [F(:,:,index),tmp] = TransferFunctionFDDT(z1,lambda(index),deltaA,deltaB,N,M,av);         %function_TransferFunction_FDDT(z1+(index-1)*dz,lambda,deltaA,N,M,av);
    else
        [F(:,:,index),tmp] = TransferFunctionASD(z1,lambda(index),deltaA,deltaB,N,M);
    end; t = t+tmp;
end
tic
for index = 1:K
