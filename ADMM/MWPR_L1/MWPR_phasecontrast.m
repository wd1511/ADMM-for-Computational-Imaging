function [out,t,RMSE] = MWPR_phasecontrast(o,u0,z1,lambda,deltaA,deltaB,ITER)
...
%          u0est=ifft2((fft2(Ar_u0)).*Sb(:,:,index));
        ...
        Ar_u0(coordy,coordx)=o(:,:,1).*exp(i*angle(Ar_u0(coordy,coordx)));
        u0est=ifft2((fft2(Ar_u0)).*Sb(:,:,1));
        u0est=abs(u0est).*exp(1i*angle(u0est)./(lambda(2)/lambda(1)));
        Ar_u0(:,:)=ifft2((fft2(u0est)).*Sf(:,:,2));
        

        Ar_u0(coordy,coordx)=o(:,:,2).*exp(1i*angle(Ar_u0));
        u0est=ifft2((fft2(Ar_u0)).*Sb(:,:,2));
        u0est=abs(u0est).*exp(1i*angle(u0est)./(lambda(3)/lambda(2)));
        Ar_u0(:,:)=ifft2((fft2(u0est)).*Sf(:,:,3));
        
        Ar_u0(coordy,coordx)=o(:,:,3).*exp(1i*angle(Ar_u0));
        u0est=ifft2((fft2(Ar_u0)).*Sb(:,:,3));
        u0est=abs(u0est).*exp(1i*angle(u0est)./(lambda(1)/lambda(3)));
        Ar_u0(:,:)=ifft2((fft2(u0est)).*Sf(:,:,1));
   
        Ar_u0(coordy,coordx)=o(:,:,1).*exp(1i*angle(Ar_u0));
        u0est=ifft2((fft2(Ar_u0)).*Sb(:,:,1));
