function [out,t,RMSE] = MWPR_average(o,u0,z1,lambda,deltaA,deltaB,ITER)
...
for jndex = 1:ITER
     if nargout > 1,    tic; end
     for index=1:K
         Ar_u0(:,:,index)=ifft2((fft2(u0est)).*Sf(:,:,index));
         Ar_u0(coordy,coordx,index)=o(:,:,index).*exp(i*angle(Ar_u0(coordy,coordx,index)));
         o_est(:,:,index)=ifft2((fft2(Ar_u0(:,:,index))).*Sb(:,:,index));
     end
     u0est=mean(o_est,3);
    if nargout > 1,   t=t+toc; end
     if cRMSE
        ...
        subplot(2,3,1),imshow(abs(out),[]), title('amplitude, average'),xlabel(['RMSE=' num2str(RMSE(1,jndex))])
        subplot(2,3,2),plot(RMSE(1,1:jndex)), title('convergence: amplitude'),ylabel('RMSE'),xlabel('# of iterations');grid on 
        
        subplot(2,3,4),imshow(angle(out)+tmp,[]), title('phase, average'), xlabel(['RMSE=' num2str(RMSE(2,jndex))])
